...
public class NFoldModelContext extends ModelContextBase {
    ...
    public void setModelsBeforeCacheUse(int modelsBeforeCacheUse) {
        ...
        //copy old data
        for (int i = 0; i < oldValid.length; i++)
            System.arraycopy(oldValid[i], 0, validIndex[i], 0, oldValid[0].length);
        for (int i = 0; i < oldLearn.length; i++)
            System.arraycopy(oldLearn[i], 0, learnIndex[i], 0, oldLearn[0].length);
