...
public class CascadeCorrelation implements ILearningAlgorithm, Serializable {
    ...
    public CascadeCorrelation(NeuralNetwork neuralNetwork, int maxLayersNumber, double acceptableError, int candNumber,
                              boolean log, int usedAlg) throws Exception {
    ...
    public void setQuickParams(int candUpdatesCycles, double minCorGrowth, double candDecay, double candMaxAlfa, double candEpsilon, int candPatienceParam,
                               int outUpdateCycles, double minErrorRed, double outDecay, double outMaxAlfa, double outEpsilon, int outPatienceParam) {
    ...
    public void setRpropParams(int candUpdateCycles, double minCorGrowth, double candDecay, double candEtaMinus, double candEtaPlus, double outEtaMinus, double outEtaPlus, int candPatienceParam,
                               int outUpdateCycles, double minErrorRed, double outDecay, int outPatienceParam) {
    ...
    private void initializeLog(String fileName) throws Exception {
        try {
            ...
            //FileWriter fstream = new FileWriter(fileName + "_" + month + "-" + day + "_" + hour + "." + minute+ "." + second + ".txt");
        ...
        try {
            for (int i = 0; i < outputs.length; i++) {
                ...
                for (int k = 0; k < outputs[i].length; k++) {
                    ...
                    if (Math.abs(outputs[i][k] - trainingSet.getTrainingPattern(i).getDesiredOutput(k)) > 0.1)
                        out.write("!!");
                    out.write("   ");
                }
                out.newLine();
    ...
    public void trainNetwork(TrainingSet trainingSet) throws Exception {
        ...
        //double [][] outputs = this.neuralNetwork.extractOutput(trainingSet, this.neuralNetwork.outputLayer().size());
        //this.writeOutputToLog(outputs, trainingSet);
    ...
    private boolean moreTrainingOutput(int numberOfEpoch, TrainingSet trainingSet) throws Exception {
        ...
        if (numberOfEpoch % this.outPatienceParam == 0) {
            ...
            if (Math.abs(this.currentError - this.lastError) <= this.minErrorRed * this.lastError) return false;
            else {
        ...
        try {
            do {
                ...
                try {
                    ...
                    //this.neuralNetwork.printError(trainingSet);
    ...
    public DataForCorrelationComp getDataForCorrelation(int candidatesNumber, int outputsNumber, TrainingSet trainingSet, NeuronLayer candidatesLayer) throws Exception {
        try {
            ...
            while (patternIterator.hasNext()) {
                ...
                //calculation of candidates value
                while (neuronIterator.hasNext()) {
