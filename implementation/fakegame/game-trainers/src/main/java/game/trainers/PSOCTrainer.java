...
import configuration.game.trainers.PSOCConfig;

/**
 * @author weberj1
 */

public class PSOCTrainer extends Trainer implements Uncmin_methods {
    ...
    public void init(GradientTrainable uni, Object cfg) {
        super.init(uni, cfg);
        PSOCConfig config = (PSOCConfig) cfg;
    ...
    protected void updateParticles(Particle[] particles, int i) {
        for (int d = 0; d < coefficients; d++) {
            // velocity
            particles[i].v[d] = chi * (particles[i].v[d] +
    ...
    public String getMethodName() {
        return "Particle Swarm Optimization - Canonical";
    }

    public Class getConfigClass() {
        return PSOCConfig.class;
    }
